...
[![npm version](https://img.shields.io/npm/v/claude_bar)](https://www.npmjs.com/package/claude_bar)
[![license](https://img.shields.io/github/license/xuyiwenak/claude_bar)](LICENSE)
...
A backup is saved as `settings.json.claude_bar_backup`.
...
```bash
npx claude_bar          # always fetches the latest from npm
```
...
原始 `settings.json` 会备份为 `settings.json.claude_bar_backup`。

### 工作原理

Claude Code 在每次对话后调用状态栏命令，通过 stdin 传入 JSON 数据。`claude_bar` 从三个来源读取信息：
...
```bash
npx claude_bar   # 每次都从 npm 拉取最新版本
```
...
[![Star History Chart](https://api.star-history.com/svg?repos=xuyiwenak/claude_bar&type=Date)](https://star-history.com/#xuyiwenak/claude_bar&Date)
